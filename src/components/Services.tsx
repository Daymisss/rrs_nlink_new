...
export function Services({ onRequestBooking }) {
  const coreServices = [
    {
      icon: Home,
  ...
  return (
    <div id="services" className="py-20 bg-gray-50 relative overflow-hidden">
      {/* Grid Background */}
      <div className="absolute inset-0 opacity-5">
        <div
          className="absolute inset-0"
          style={{
            backgroundImage: `linear-gradient(hsl(var(--grid-color)) 1px, transparent 1px), linear-gradient(90deg, hsl(var(--grid-color)) 1px, transparent 1px)`,
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        {/* Core Services */}
        <div id="services-core" className="text-center mb-16">
          <h2 className="text-3xl sm:text-4xl font-bold text-gray-900 mb-4">Our Core Services</h2>
          ...
          {coreServices.map((service, index) => {
            ...
            return (
              <div
                id={slug}
                key={index}
                className="group overflow-hidden hover:shadow-2xl transition-all duration-500 transform hover:-translate-y-2 bg-white border border-gray-200 rounded-xl"
              >
                ...
                <div className="p-6">
                  ...
                  <button type="button" onClick={() => onRequestBooking?.(service.title, 'core')} className="w-full btn-primary py-3 px-4 rounded-lg transform hover:scale-105 transition-all duration-200 font-semibold">
                    Get Quote
                  </button>
                </div>
              </div>
        ...
        {/* Specialty Services */}
        <div id="services-specialty" className="text-center mb-16">
          <h2 className="text-3xl sm:text-4xl font-bold text-gray-900 mb-4">Specialty Services</h2>
          <p className="text-xl text-gray-600 max-w-3xl mx-auto">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
          {specialtyServices.map((service, index) => {
            ...
            const slug = slugify(service.title)
            return (
              <div key={index} id={slug} className="group overflow-hidden hover:shadow-2xl transition-all duration-500 transform hover:-translate-y-2 bg-white border border-gray-200 rounded-xl">
                <div className="relative overflow-hidden">
                  <img src={service.image} alt={service.title} className="w-full h-40 object-cover group-hover:scale-110 transition-transform duration-500" />
                  <div className="absolute top-4 right-4 bg-orange-500 text-white px-3 py-1 rounded-full text-sm font-medium">{service.price}</div>
                ...
                <div className="p-6">
                  ...
                  <p className="text-gray-600 mb-4">{service.description}</p>
                  <button type="button" onClick={() => onRequestBooking?.(service.title, 'specialty')} className="w-full btn-primary py-3 px-4 rounded-lg transform hover:scale-105 transition-all duration-200 font-semibold">Book Service</button>
                </div>
              </div>
            )
          })}
